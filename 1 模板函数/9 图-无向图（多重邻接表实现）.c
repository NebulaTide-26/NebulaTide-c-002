...
udge_node** udgraph_get_edgelist(intudgraph *graph);						// 获取边节点数组（按权值升序排列）
udge_node** udgraph_MST_Prim(intudgraph *graph);							// 求最小生成树（Prim算法）
udge_node** udgraph_MST_Kruskal(intudgraph *graph);							// 求最小生成树（Kruskal算法）
int_shortest_road* udgraph_shortest_path_Dijkstra(intudgraph *graph, udgv_node *start);
																			// 求最短路径（Dijkstra算法，计算从start到所有其他节点的最短路径）
int_shortest_road** udgraph_shortest_path_Floyd(intudgraph *graph);			// 求最短路径（Floyd算法，计算图中任意两点的最短路径）
...
/* 求最小生成树（Prim算法）
   返回值：被选入树中的边指针的数组，按weight从小到大排列
...
/* 求最短路径（Dijkstra算法，计算从start到所有其他节点的最短路径）
   返回值：int_shortest_road数组（该数组的序号与实际顶点序号无关）
...
int_shortest_road* udgraph_shortest_path_Dijkstra(intudgraph *graph, udgv_node *start) {
	...
	int_shortest_road *out = (int_shortest_road*)malloc(sizeof(int_shortest_road) * graph->size_v);
	if(!out) {
		if (udgraph_error_output) printf("[Memory exceeded] There is not enough space to count the shortest road.\n");
		free(list);
		free(distance);
...
/* 求最短路径（Floyd算法，计算图中任意两点的最短路径）
   返回值：二维int_shortest_road数组（该数组的序号与实际顶点序号无关）
...
*/
int_shortest_road** udgraph_shortest_path_Floyd(intudgraph *graph) {
	if(graph == NULL) {
		if(udgraph_error_output) printf("[Invalid function argument] The target udgraph pointer is NULL.\n");
		return NULL;
	}
	...
	for(int i = 0; i < graph->size_v; i++) {
		out[i] = (int_shortest_road*)malloc(sizeof(int_shortest_road) * graph->size_v);
		if(!out[i]) {
			if (udgraph_error_output) printf("[Memory exceeded] There is not enough space to count the shortest road.\n");
			for(int j = 0; j < i; j++) {
				free(out[j]);
