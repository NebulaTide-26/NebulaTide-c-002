...
基本信息类型：intbitree
节点类型：binode
若同时建立多个二叉树，需建立多个node变量类型 和 多个基本信息变量，并将所有函数的node分别换成对应词（并设定平行的不同函数名称）
...
} binode_level;// 层序遍历结构体

typedef struct {
	binode_level *head;
	binode_level *tail;
	int count;
} intlevel_bi;// 层序遍历总目录结构体（采用链表结构）



static intlevel_bi _level_bi = {NULL, NULL, 0};
#define level_bi (&_level_bi)										// 全局变量level_bi：层序遍历总目录

static int bitree_error_output = 1; 								// 全局变量bitree_error_output：控制是否打印错误信息（默认打印）
...
//int bitree_printtree(intbitree *tree);								// 打印一整棵树（填写DIY_printnode时尽量只取某一数字成员）（复杂且不急用，待完善）
...
/* 中序遍历（左根右）（根据当前接收的节点，输出下一个节点地址）
   ...
   注意：由于不是从根节点开始遍历，完整遍历时需要先手动（调用bitree_traverse_in_start）定位到中序首节点处
*/
...
/* 后序遍历（左右根）（根据当前接收的节点，输出下一个节点地址）
   ...
   注意：由于不是从根节点开始遍历，完整遍历时需要先手动（调用bitree_traverse_post_start）定位到后序首节点处
*/
...
//   返回值：1（空树等错误返回-1）
//*/
//int bitree_printtree(intbitree *tree) {
//	if(tree==NULL) {
//		if (bitree_error_output) printf("[Invalid function argument] The target bitree pointer is NULL.\n");
//		return -1;
//	}
